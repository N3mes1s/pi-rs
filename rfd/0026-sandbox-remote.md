...
- **Implemented:**
  ...
  - **v2 contextfs RW /work on Sprites** — heavily progressed; one upstream bug remaining:
    ...
    - **Phase C3** (Sprites/Agora e2e): integration test `sprites_contextfs_e2e.rs` (`8b48b04`) provisions a real Sprite, runs the full bootstrap with `Transport::Agora`. The upstream `cfs-mesh receive-uds` CLI bug (`into_child()` consumed the `ListenerHandle`, dropping the agora identity `TempDir` before the still-running listener subprocess read `rooms.json`) **is fixed on contextfs branch `rfd/0029`** (tip `2e04567`, 2026-05-18): both `expose_uds_cli` and `receive_uds_cli` now block via `ListenerHandle::wait()`, which keeps the identity dir alive for the subprocess's lifetime. The loopback regression `contextfs_loopback_rw_mount.rs` re-verified green against that tip (2026-08-06). Remaining: one `SPRITES_TOKEN`-bound run of `sprites_contextfs_e2e.rs` to confirm the Agora path end-to-end.
