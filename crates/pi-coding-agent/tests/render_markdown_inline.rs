...
fn markdown_bold_renders_without_asterisks() {
    ...
    assert!(all_text.contains("bold text"), "result should contain 'bold text': {}", all_text);
    assert!(!all_text.contains("**"), "result should not contain asterisks: {}", all_text);
}
...
fn markdown_italic_renders_without_asterisks() {
    ...
    assert!(all_text.contains("italic text"), "result should contain 'italic text': {}", all_text);
    assert!(!all_text.contains("_"), "result should not contain underscores: {}", all_text);
}
...
fn markdown_inline_code_renders_without_backticks() {
    ...
    assert!(all_text.contains("println!"), "result should contain 'println!': {}", all_text);
    assert!(!all_text.contains("`"), "result should not contain backticks: {}", all_text);
}
...
fn markdown_combined_styling_no_literal_markers() {
    ...
    assert!(!all_text.contains("**") && !all_text.contains("_") && !all_text.contains("`"),
            "should have no literal markers: {}", all_text);
}
...
fn markdown_bold_spans_have_accent_color() {
    ...
    assert!(!bold_spans.is_empty(), "should find spans containing 'bold' or 'text'");
    assert!(
