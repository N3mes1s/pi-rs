...
fn transcript_assistant_text_renders_markdown_bold_inline() {
    ...
    assert!(all_text.contains("bold"), "should contain 'bold': {}", all_text);
    assert!(all_text.contains("answer"), "should contain 'answer': {}", all_text);
    assert!(!all_text.contains("**"), "should not contain literal asterisks: {}", all_text);
    
    // Should have "pi>" prefix
    assert!(all_text.contains("pi>"), "should have 'pi>' prefix: {}", all_text);
}
...
fn transcript_assistant_text_renders_markdown_inline_code() {
    ...
    assert!(all_text.contains("println!"), "should contain 'println!': {}", all_text);
    assert!(!all_text.contains("`"), "should not contain backticks: {}", all_text);
}
...
fn transcript_assistant_text_with_fenced_code_block() {
    ...
    assert!(all_text.contains("main"), "should contain 'main': {}", all_text);
    assert!(all_text.contains("rust"), "should contain 'rust' language label: {}", all_text);
    
    // Should contain intro and outro text
    assert!(all_text.contains("Here is a function"), "intro text: {}", all_text);
