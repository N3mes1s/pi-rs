...
fn code_block_includes_language_label() {
    ...
    assert!(
        label_text.contains("rust"),
        "first line should show 'rust' language label"
    );
}
...
fn code_block_renders_non_empty_lines() {
    ...
    assert!(
        result.len() >= 3,
        "code block should have header, body, and footer"
    );
    ...
    assert!(
        all_text.contains("main") || all_text.contains("fn"),
        "body should contain code content"
    );
}
...
fn code_block_empty_language_defaults_to_code() {
    ...
    assert!(
        label_text.contains("code"),
        "empty language should show 'code' label"
    );
}
...
fn code_block_has_border_lines() {
    ...
    assert!(
        first_text.contains("╭"),
        "header should have opening border"
    );
...
fn code_block_preserves_indentation() {
    ...
    let body_lines = result.iter().skip(1).take(result.len().saturating_sub(2));

    for line in body_lines {
        if !line.spans.is_empty() {
            let first_span = &line.spans[0];
            assert!(
                first_span.text.starts_with("  "),
                "code line should be indented with '  ': {:?}",
                first_span.text
            );
        }
    }
