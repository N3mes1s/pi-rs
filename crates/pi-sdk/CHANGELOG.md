...
- `crates/pi-sdk/compatibility.toml` + generated `COMPATIBILITY.md` (Commit G). `scripts/gen-compatibility-matrix.sh` regenerates the markdown.
- `.github/workflows/pi-sdk-supply-chain.yml`: cargo-audit, cargo-deny, pi-sdk-canary, examples-build, doc-tests, COMPATIBILITY-up-to-date (Commit I). cargo-semver-checks gate scaffolded; enabled in Commit J's PR after first crates.io publish.
